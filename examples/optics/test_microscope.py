...
#### 600 – Interactive Ray Tracing with Qt5 (sliders + spin boxes)
if QT5:
    # =============================================================================
    # 600 – Interactive Ray Tracing with Qt5 (sliders + spin boxes)
    # =============================================================================
    ...
    if "ray_trace" in sets.flow:
        print("\n" + "="*60)
        print("600 · Interactive Ray Tracing with Qt5 (Sliders + Spin Boxes)")
        print("="*60)
        print("A Qt5 window will open with spin boxes and sliders for each parameter.")
        print("Adjust any control to see the rays and image height update in real time.")
        ...
        def default_object_distance(f1, E):
            """Object distance placing the intermediate image at v = f1 + E."""
            return 1.0 / (1.0/f1 - 1.0/(f1 + E))

        def compute_system(f1, f2, E, g, d_n=250.0):
            d = f1 + E + f2
            M_obj = -(f1 + E) / g
            M_eye = d_n / f2
            M_total = M_obj * M_eye
            ...
            Td = T_mat(d)
            S_dbl = L2 @ Td @ L1
            return d, M_obj, M_eye, M_total, S_dbl
        ...
        def trace_microscope(f1, f2, E, h_obj, g, d_n=250.0):
            d, M_obj, M_eye, M_total, S_dbl = compute_system(f1, f2, E, g, d_n)
            steps = [
                (T_mat(g),      g),
                (L_mat(f1),     0.0),
                (T_mat(d),      d),
        ...
        def compute_image_distance(f1, f2, E, g):
            """Compute the image distance b from the eyepiece using s12=0."""
            d = f1 + E + f2
            # M = L2 @ T(d) @ L1 @ T(g)
            M = L_mat(f2) @ T_mat(d) @ L_mat(f1) @ T_mat(g)
            # S = T(b) @ M => s12 = M[0,1] + b * M[1,1] = 0 => b = -M[0,1]/M[1,1]
            b = -M[0, 1] / M[1, 1]
            return b
        ...
        class MicroscopeGUI(QMainWindow):
            def __init__(self):
                super().__init__()
                self.setWindowTitle("Compound Microscope – Interactive Ray Tracing")
                ...
                self.init_E  = 160.0
                self.init_h  = 0.1
                self.init_g  = default_object_distance(self.init_f1, self.init_E)
                ...
                # Helper to create a row: label + spin box + slider
                def create_param_row(label_text, obj, min_val, max_val, step, init_val):
                    row = QHBoxLayout()
                    ...
                    spin.valueChanged.connect(obj.update_plot)

                    slider = QSlider(Qt.Horizontal)
                    slider.setRange(0, 1000)
                    # Map slider value to spin value
                    def slider_to_spin(val):
                        spin.blockSignals(True)
                        spin_val = min_val + (max_val - min_val) * (val / 1000.0)
                        spin.setValue(spin_val)
                        spin.blockSignals(False)
                        # Explicitly trigger update since spin signals were blocked
                        obj.update_plot()
                ...
                # Create rows and store spin boxes and sliders as attributes
                row, self.spin_f1, self.slider_f1 = create_param_row(
                    "f₁ (mm):", self, 1.0, 100.0, 0.5, self.init_f1)
                control_layout.addLayout(row)

                row, self.spin_g, self.slider_g = create_param_row(
                    "g (mm):", self, 1.0, 110.0, 0.1, self.init_g)
                control_layout.addLayout(row)

                row, self.spin_f2, self.slider_f2 = create_param_row(
                    "f₂ (mm):", self, 10.0, 100.0, 1.0, self.init_f2)
                control_layout.addLayout(row)

                row, self.spin_E, self.slider_E = create_param_row(
                    "E (mm):", self, 100.0, 300.0, 5.0, self.init_E)
                control_layout.addLayout(row)

                row, self.spin_h, self.slider_h = create_param_row(
                    "h_obj (mm):", self, 0.01, 1.0, 0.01, self.init_h)
                control_layout.addLayout(row)
            ...
            def update_plot(self, value=None):
                ...
                # Enforce physical constraint: g >= f1
                if g < f1:
                    g = f1
                ...
                # Compute ray traces
                z_marg, y_marg, z_chief, y_chief, M_total, g_used, d = trace_microscope(
                    f1, f2, E, h, g
                )
                image_height = M_total * h
                M_classical = -(E * self.d_n) / (f1 * f2)

                # Compute image distance b
                b = compute_image_distance(f1, f2, E, g_used)
                ...
                # Plot rays
                self.ax.plot(z_marg, y_marg, color='blue', linewidth=2.0,
                             marker='o', markersize=4, label='Marginal ray')
                self.ax.plot(z_chief, y_chief, color='red', linewidth=2.0,
                             marker='s', markersize=4, label='Chief ray')

                # Optical axis
                self.ax.axhline(0, color='gray', linewidth=0.8, linestyle='--', alpha=0.7)

                # Lens planes
                lens_z = [g_used, g_used + d]
                lens_labels = [f"Objective L₁\n(f₁={f1:.1f} mm)",
                               f"Eyepiece L₂\n(f₂={f2:.1f} mm)"]
                ymax = max(abs(y_marg.max()), abs(y_marg.min()),
                           abs(y_chief.max()), abs(y_chief.min()))
                ymax = max(ymax, 0.5) * 1.2
                for lz, ll in zip(lens_z, lens_labels):
                    self.ax.axvline(lz, color='green', linewidth=1.5, linestyle=':', alpha=0.8)
                    self.ax.text(lz, ymax*0.95, ll, ha='center', va='bottom',
                                 fontsize=8.5, color='green')

                # Intermediate image plane
                z_inter = g_used + f1 + E
                self.ax.axvline(z_inter, color='orange', linewidth=1.0, linestyle='--', alpha=0.5)

                # Info box – now includes b
                info_text = (f"f₁ = {f1:.2f} mm  |  f₂ = {f2:.2f} mm  |  E = {E:.1f} mm\n"
                             f"g = {g_used:.2f} mm  |  d = {d:.2f} mm  |  b = {b:.2f} mm\n"
                             f"M_total = {M_total:.2f}×  |  Object height = {h:.3f} mm  |  Image height = {image_height:.3f} mm")
                ...
                # Labels and style
                self.ax.set_xlabel("z  [mm]", fontsize=12, color='black')
                self.ax.set_ylabel("Ray height y  [mm]", fontsize=12, color='black')
                self.ax.set_title(f"Compound Microscope – Interactive Ray Tracing\n"
                ...
                self.ax.legend(fontsize=11, facecolor='white', edgecolor='gray')
...
#### 600 – Interactive Ray Tracing with Sliders (replaces static plot)        
    ...
    if "ray_trace" in sets.flow:
        ...
        INIT_E  = 160.0
        INIT_h  = 0.1
        INIT_g  = default_object_distance(INIT_f1, INIT_E)   # object distance g (>= f1)
        d_n = 250.0
        ...
        plt.subplots_adjust(bottom=0.42)          # space for sliders
        ...
        # g is given its own individual slider, separate from the other distances.
        # Physical constraint:  g >= f1  (object must lie at/behind the objective
        # front focal point).  This is enforced dynamically in update_plot().
        slider_config = [
            ('f1', 'f₁ (mm)',    0.32, 1.0, 100.0, 0.5,  INIT_f1),
            ('g',  'g (mm)',     0.26, 1.0, 110.0, 0.1,  INIT_g),
            ('f2', 'f₂ (mm)',    0.20, 10.0, 100.0, 1.0, INIT_f2),
            ('E',  'E (mm)',     0.14, 100.0, 300.0, 5.0, INIT_E),
            ('h',  'h_obj (mm)', 0.08, 0.01, 1.0, 0.01,  INIT_h),
        ]
        ...
        # Update function
        # ------------------------------------------------------------------
        # Guard flag to avoid recursive callbacks when we programmatically
        # correct the g slider to satisfy the constraint g >= f1.
        ...
        def update_plot(val):
            ...
            # Enforce the physical constraint  g >= f1.
            if g < f1:
                g = f1
                _updating['flag'] = True
                sliders['g'].set_val(g)   # snap the slider back to g = f1
                _updating['flag'] = False
    
            ax.clear()
            ax.set_facecolor('white')
    
            z_marg, y_marg, z_chief, y_chief, M_total, g, d = trace_microscope(f1, f2, E, h, g)
