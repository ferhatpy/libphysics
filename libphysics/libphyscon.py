...
# Copyright 2004,2007,2010 Herman J.C. Berendsen, <www.hjcb.nl/python>;
# email <herman@hjcb.nl>. The data originate from the CODATA 2010 release,
...
# Some improvements have been done by Ferhat Nutku @ 2015.
"""
Alternatively scipy.constants.physical_constants library can be used.
from scipy import constants
constants.m_e -> 9.1093837015e-31
from scipy.constants import find, physical_constants
physical_constants['electron mass'] -> (9.1093837015e-31, 'kg', 2.8e-40)
"""
...
all={'lightvel':['velocity of light in vacuum','c',cloc,0., 0.,'299 792 458(ex) m/s', 'CODATA 2010'],
     ...
     'diraceV':["Dirac's constant in eV s = h/(2 pi)",'hbareV',6.582118813e-16,5.3e-42,5.0e-8,'6.582 118 813 e-16 eV s', 'DERIVED CODATA 2006 '], # Nutku F.
     ...
     'boltzmanneV':['Boltzmann constant in eV/K ','k_BeV',8.61734255962e-05,2.4e-29,1.7e-6,'8.617 342 559 (62) eV/K','DERIVED CODATA 2006'], # Nutku F.
...
__all__ = [
    'alpha', 'a_0', 'c', 'e', 'eps_0', 'F', 'G', 'g_e', 'g_p', 'gamma_p', 'h', 'hbar', 'hbareV', 'k', 'k_B', 'k_BeV',
    'm_d', 'm_e', 'm_n', 'm_p', 'mu_B', 'mu_e', 'mu_N', 'mu_p', 'mu_0', 'N_A', 'R', 'sigma', 'u', 'G0',
    'value', 'sd', 'relsd', 'descr', 'help'
]
...
G0 = float(e)**2 / (2 * float(pi)**2 * float(hbar))  # 1.23314716001e-05, Nutku F.
...
def value(key: str) -> float:
    return float(all[key][2])

def sd(key: str) -> float:
    return float(all[key][3])

def relsd(key: str) -> float:
    return float(all[key][4])

def descr(key: str) -> None:
...
# todo:. units must be written in a dictionary.
