...
#### ABCD
        class ABCD(branch):
            ...
            def __init__(self):
                ...
                self.lambda1 = Eq(Symbol('lambda_1'), n1*alpha1)
                self.lambda2 = Eq(Symbol('lambda_2'), n2*alpha2)
                self.P       = Eq(Symbol('P'), (n2-n1)/R1)             # (29)
                self.P1      = Eq(Symbol('P_1'), (n-1)/R1)             # ~(50)
                self.P2      = Eq(Symbol('P_2'), (1-n)/R2)             # ~(50)
                
                self.T = lambda D1=D1, n1=n1: Eq(Symbol('T'), 
                            UnevaluatedExpr(Matrix(((1,    0),
                                                    (D1/n1, 1)))))  # (21)
                self.R = lambda n2=n2, n1=n1, R1=R1: Eq(Symbol('R'), 
                            UnevaluatedExpr(Matrix(((1, -(n2-n1)/R1),
                                                    (0,  1)))))     # (31)
                self.SM = Eq(Symbol('SM'), 
...
#### ----> Single Spherical Refracting Surface
                class single_spherical_refracting_surface(branch):
                    ...
                    def __init__(self, parent):
                        ...
                        self.M = Eq(Symbol('m'), n1*D2/(n2*D1))
...
#### ----> Coaxial Optical System
                class coaxial_optical_system(branch):
                    ...
                    def __init__(self, parent):
                        ...
                        self.M = Eq(Symbol('M'), c - a*D2) # (45) Magnification
                        self.matrix = \
                            Eq(Matrix(((lambda2),
                                       (x2))),
...
#### ----> Thick Lens
                class thick_lens(branch):
                    ...
                    def __init__(self, parent):
                        ...
                        self.system_matrixP1P2 = Eq(Symbol('SM'),
                            MatMul(parent.R(1,n,R2).rhs.doit(), parent.T(t,n).rhs.doit(), parent.R(n,1,R1).rhs.doit())
                            ) # (51)
...
#### ----> Thin Lens
                class thin_lens(branch):
                    ...
                    def __init__(self, parent):
                        ...
                        self.f = self.focal_length
                        self.system_matrix = Eq(Symbol('SM'),
...
#### Fiber Bragg Grating
        class Fiber_Bragg_Grating(branch):
            ...
            def __init__(self):
                ...
                self.R = self.Reflectance = Eq( Symbol('R'), kappa**2*sinh(alpha*L)**2 / (kappa**2*cosh(alpha*L)**2 - Gamma**2/4) ) # Ghatak2009 Appendix C Eq.3
                self.R_peak = Eq(Symbol('R_peak'), tanh(pi*Delta_n*L/lambda_B)**2)
