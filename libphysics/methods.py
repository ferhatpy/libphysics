...
# Differential position element
dl = 1*C.i + 1*C.j + 1*C.k
...
#        Gw        = Lambda((w), Function('Gtilde')(w))    # Callable Green's tilde function.
    ...
    def __init__(self, class_type='default'):
        ...
        if self.class_type == "default":
            ...
            #----> Laplace Transform
            self.L = self.Laplace_transform = Lambda( (f,s,t), Integral(f*exp(-s*t), (t, Integer(0), oo)) )
            ...
            self.dot_product = Eq(Symbol('C'), self.vA.dot(self.vB))
            self.dot_product_theta = Eq(Symbol('C'), self.vA.magnitude()*self.vB.magnitude()*cos(theta))
            self.cross_product = Eq(self.vC, self.vA.cross(self.vB))
            self.cross_product_theta = Eq(Symbol('C'), self.vA.magnitude()*self.vB.magnitude()*sin(theta))
            self.triple_scalar_product = Eq(Symbol('D'), self.vA.dot(self.vB.cross(self.vC)))
